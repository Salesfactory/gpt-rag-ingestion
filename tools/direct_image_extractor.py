...
fitz = _load_optional_module(
    "fitz", "PyMuPDF not available - PDF image extraction disabled"
)
PYMUPDF_AVAILABLE = fitz is not None

_pil_image_module = _load_optional_module(
    "PIL.Image", "Pillow not available - image processing disabled"
)
...
class DirectImageExtractor:
    ...
    def _extract_from_docx(self, file_bytes: bytes) -> List[Dict[str, Any]]:
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(file_bytes), "r") as zip_file:
                ...
                image_files = [
                    f for f in zip_file.namelist() if f.startswith("word/media/")
                ]
                ...
                for idx, img_path in enumerate(image_files):
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to extract image {img_path}: {str(e)}"
                        )
                        continue
            ...
            with zipfile.ZipFile(io.BytesIO(file_bytes), "r") as zip_file:
                ...
                for slide_index, slide_path in enumerate(slide_files, start=1):
                    try:
                        ...
                        self.logger.warning(
                            f"Slide file {slide_path} missing from PPTX"
                        )
                        continue

                    rels_path = slide_path.replace("slides/", "slides/_rels/").replace(
                        ".xml", ".xml.rels"
                    )
                        ...
                        try:
                            ...
                            for rel in rels_root.findall(f"{rel_ns}Relationship"):
                                ...
                                resolved = os.path.normpath(
                                    os.path.join(os.path.dirname(slide_path), target)
                                )
                                rels_map[rel_id] = resolved.replace("\\", "/")
                        except Exception as rel_error:
                            self.logger.warning(
                                f"Failed to parse relationships for {slide_path}: {rel_error}"
                            )
                    ...
                    for blip in slide_xml.findall(".//a:blip", namespaces):
                        ...
                        if (
                            media_path
                            and media_path.startswith("ppt/")
                            and media_path not in seen_media
                        ):
                    ...
                    for media_path in slide_images:
                        try:
                            ...
                            self.logger.warning(
                                f"Image {media_path} referenced by {slide_path} missing in archive"
                            )
                            continue
    ...
    def convert_to_normalized_format(
        self, extracted_images: List[Dict[str, Any]], source_url: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        for img in extracted_images:
            try:
                ...
                self.logger.warning(
                    f"Failed to normalize image {img.get('image_id', 'unknown')}: {str(e)}"
                )
                continue
