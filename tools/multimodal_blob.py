...
class MultimodalBlobClient:
    ...
    def __init__(self):
        ...
        """
        self.images_container = os.getenv(
            "IMAGES_CONTAINER_NAME", "ragindex-test-images"
        )
        ...
        if not account_name:
            logging.warning(
                "[multimodal_blob] AZURE_STORAGE_ACCOUNT not provided. Image storage will be disabled."
            )
            return
        ...
        try:
            ...
            logging.info(
                "[multimodal_blob] Initialized with managed identity authentication."
            )
        except Exception as e:
            logging.error(
                f"[multimodal_blob] Failed to initialize blob service client: {e}"
            )
            logging.info(
                "[multimodal_blob] This may be due to DNS resolution or authentication issues in local development."
            )
    ...
    def _ensure_container_exists(self):
        ...
        try:
            ...
            logging.debug(
                f"[multimodal_blob] Container '{self.images_container}' exists."
            )
            return True
        except Exception:
            try:
                ...
                logging.info(
                    f"[multimodal_blob] Created container '{self.images_container}'."
                )
                return True
            except ResourceExistsError:
                logging.debug(
                    f"[multimodal_blob] Container '{self.images_container}' already exists."
                )
                return True
            except Exception as e:
                logging.error(
                    f"[multimodal_blob] Failed to create container '{self.images_container}': {e}"
                )
                self.initialized = False
                return False
    ...
    def _sanitize_metadata_value(self, value: Any) -> str:
        """
        Sanitize metadata values to ensure they are ASCII-only for Azure Blob Storage.

        Azure Blob Storage metadata must contain only ASCII characters (Latin-1 encoding).
        This method converts any value to an ASCII-safe string.
        ...
        # This prevents 'latin-1' codec errors when storing metadata
        try:
            ...
            sanitized = value.encode("ascii", errors="replace").decode("ascii")
            logging.debug(
                "[multimodal_blob] Sanitized metadata value containing non-ASCII characters"
            )
            return sanitized

    def _generate_image_path(
        self, document_url: str, image_id: str, image_format: str = "jpg"
    ) -> str:
        """
    ...
    def store_image(
        ...
        if not self.initialized:
            logging.warning(
                f"[multimodal_blob] Blob storage not available, skipping image {image_id} storage"
            )
            return {
                "success": False,
        ...
        try:
            ...
            # Prepare metadata - all values must be ASCII-safe for Azure Blob Storage
            blob_metadata = {
                "source_document": self._sanitize_metadata_value(document_url),
                "image_id": self._sanitize_metadata_value(image_id),
                "content_type": "image/jpeg",
                "extraction_timestamp": str(int(time.time())),
            }

            if organization_id:
                blob_metadata["organization_id"] = self._sanitize_metadata_value(
                    organization_id
                )
            ...
            if location_metadata:
                ...
                if bounding_polygons is not None:
                    try:
                        # Serialize to JSON with ASCII encoding
                        bounding_polygons_value = json.dumps(
                            bounding_polygons, ensure_ascii=True
                        )
                    except (TypeError, ValueError):
                        # Fallback: convert to string and sanitize
                        bounding_polygons_value = self._sanitize_metadata_value(
                            bounding_polygons
                        )
                else:
                ...
                blob_metadata.update(
                    {
                        "page_number": self._sanitize_metadata_value(
                            location_metadata.get("pageNumber", 0)
                        ),
                        "ordinal_position": self._sanitize_metadata_value(
                            location_metadata.get("ordinalPosition", 0)
                        ),
            ...
            blob_client = self.blob_service_client.get_blob_client(
                container=self.images_container, blob=image_path
            )
            ...
            # Generate public URL
            image_url = (
                f"{self.storage_account_url}/{self.images_container}/{image_path}"
            )
    ...
    def store_images_batch(
        ...
        logging.info(
            f"[multimodal_blob] Storing {len(images)} images for document {document_url}"
        )
        ...
        for img in images:
            ...
            if not image_data:
                logging.warning(
                    f"[multimodal_blob] No image data for {image_id}, skipping"
                )
                results.append(
                    {
                        "success": False,
        ...
        logging.info(
            f"[multimodal_blob] Stored {success_count}/{len(results)} images successfully"
        )
        ...
        try:
            ...
            for blob in blobs:
                try:
                    ...
                    logging.warning(
                        f"[multimodal_blob] Failed to delete blob {blob.name}: {e}"
                    )

            logging.info(
                f"[multimodal_blob] Deleted {deleted_count} images for document {document_url}"
            )
